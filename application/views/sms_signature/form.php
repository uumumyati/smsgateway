...
<?= form_open($formAction, ['class' => 'form-horizontal', 'role' => 'form']) ?>
  ...
  <!-- is_signature -->
  <div class="form-group has-feedback <?= setValidationStyle('is_signature') ?>">
    <div class="col-sm-offset-2 col-sm-5">
      <div class="checkbox">
        <label>
          <?= form_checkbox('is_signature', true, isset($input->is_signature) && ($input->is_signature == true) ? true : false) ?> Signature
        </label>
      </div>
    </div>
    <?php if (form_error('is_signature')) : ?>
    <div class=" col-sm-5">
        <?= form_error('is_signature');?>
    </div>
    <?php endif ?>
  </div>
